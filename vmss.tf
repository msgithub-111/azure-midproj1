resource "azurerm_virtual_machine_scale_set" "user23vmss" {
name = var.vmss
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user23subnet1.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user23beapool.id]
