resource "azurerm_virtual_machine_scale_set" "user23vmss2" {
name = var.vmss2
...
os_profile {
    computer_name_prefix = "user23vmb"
    admin_username = "msheo"  ## VM 에 접속할 계정
...
ssh_keys {
    path = "/home/msheo/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user01 등 
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user23subnet2.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user23beapool2.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool2.id]
    }
        network_security_group_id = azurerm_network_security_group.user23nsg.id
}
tags = {
    environment = "staging"
    }
